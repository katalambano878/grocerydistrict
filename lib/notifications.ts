...
const ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'admin@grocerydistrict.shop';
const EMAIL_FROM = process.env.EMAIL_FROM || 'Grocery District <noreply@grocerydistrict.shop>';
const BRAND = {
    name: 'Grocery District',
    color: '#2B2C86',
    colorLight: '#f9fafb',
    colorDark: '#1A1B5E',
    url: (process.env.NEXT_PUBLIC_APP_URL || (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : null) || 'https://grocerydistrict.shop').replace(/\/+$/, ''),
    phone: process.env.CONTACT_PHONE || '+233552971577',
};
...
export async function sendSMS({ to, message }: { to: string; message: string }) {
    ...
    try {
        ...
        const response = await fetch('https://api.moolre.com/open/sms/send', {
            ...
            body: JSON.stringify({
                type: 1,
                senderid: process.env.SMS_SENDER_ID || 'GroceryDist',
                messages: [
                    {
                        recipient: recipient,
                        message: message
...
export async function sendOrderConfirmation(order: any) {
    ...
    const baseUrl = (process.env.NEXT_PUBLIC_APP_URL || (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : null) || 'https://grocerydistrict.shop').replace(/\/+$/, '');
...
`, `New order #${order_number} from ${name}`);
    ...
    if (phone) {
        const smsMessage = trackingNumber
            ...
            : `Hi ${name}, your order #${order_number || id} at Grocery District is confirmed! Track here: ${trackingUrl}${shippingNotesSms}`;
...
`, `Welcome to ${BRAND.name}, ${firstName}!`)
    ...
    if (phone) {
        await sendSMS({
            to: phone,
            message: `Welcome ${firstName}! Thanks for joining Grocery District.`
        });
    }
}
