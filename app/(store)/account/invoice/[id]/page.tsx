  ...
  if (loading) {
    return (
      <main className="min-h-screen flex items-center justify-center">
        <i className="ri-loader-4-line animate-spin text-3xl text-[#2B2C86]" />
      </main>
    );
  }
  ...
  if (error || !order) {
    return (
      <main className="min-h-screen flex flex-col items-center justify-center px-4 text-center">
        ...
        <Link href="/account?tab=orders" className="text-[#2B2C86] font-semibold">
          Back to orders
        </Link>
      </main>
    );
      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-8 print:hidden">
          ...
          <button
            ...
            className="px-4 py-2 bg-[#2B2C86] text-white rounded-lg font-semibold"
          >
            Print / Save PDF
          </button>
        </div>
        ...
        <div className="border border-gray-200 rounded-xl p-8 print:border-0 print:p-0">
          <div className="flex items-start justify-between gap-4 mb-8">
            <div>
              <p className="text-2xl font-bold tracking-wide text-[#2B2C86]">Grocery District</p>
              <p className="text-sm text-gray-600">UK &amp; USA groceries in Accra</p>
            </div>
            ...
            <tbody>
              {items.map((item: any) => (
                <tr key={item.id} className="border-b border-gray-100">
                  ...
                  <td className="py-3 pr-2">GH₵{money(item.unit_price)}</td>
                  <td className="py-3 text-right">
                    GH₵{money(item.total_price ?? asNumber(item.unit_price) * item.quantity)}
                  </td>
                </tr>
              ))}
            </tbody>
