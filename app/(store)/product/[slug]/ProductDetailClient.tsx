...
export default function ProductDetailClient({ slug }: { slug: string }) {
  ...
  const breadcrumbSchema = generateBreadcrumbSchema([
    { name: 'Home', url: 'https://grocerydistrict.shop' },
    { name: 'Shop', url: 'https://grocerydistrict.shop/shop' },
    { name: product.category, url: `https://grocerydistrict.shop/shop?category=${product.category.toLowerCase().replace(/\s+/g, '-')}` },
    { name: product.name, url: `https://grocerydistrict.shop/product/${slug}` }
  ]);
              ...
              <div>
                <div className="flex items-start justify-between mb-4">
                  ...
                  <button
                    ...
                    className="w-12 h-12 flex items-center justify-center border-2 border-gray-200 hover:border-[#2B2C86] rounded-full transition-colors cursor-pointer"
                  >
                ...
                <div className="flex items-center mb-6">
                  <div className="flex items-center space-x-1 mr-3">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <i
                        key={star}
                        className={`${star <= Math.round(product.rating) ? 'ri-star-fill text-[#E6308A]' : 'ri-star-line text-gray-300'} text-lg`}
                      ></i>
                    ))}
                  </div>
                ...
                <div className="flex items-baseline space-x-4 mb-6">
                  {hasVariants && !selectedVariant ? (
                    ...
                    <span className="text-3xl lg:text-4xl font-bold text-gray-900"><Price amount={activePrice} /></span>
                  )}
                  {product.compare_at_price && product.compare_at_price > activePrice && (
                    <span className="text-xl text-gray-400 line-through"><Price amount={product.compare_at_price} /></span>
                  )}
                </div>
                            ...
                            return (
                              <button
                                ...
                                className={`relative group flex items-center gap-2 px-3 py-2 rounded-xl border-2 transition-all duration-150 cursor-pointer select-none
                                  ${isSelected
                                    ? 'border-[#2B2C86] bg-[#2B2C86] text-white shadow-md scale-105'
                                    : isOutOfStock
                      ...
                      return (
                        <div>
                          <div className="flex items-center justify-between mb-2">
                            ...
                            {selectedVariant
                              ? <span className="text-sm font-semibold text-gray-900"><Price amount={selectedVariant.price || product.price} /></span>
                                ...
                                return (
                                  <button
                                    ...
                                    className={`relative rounded-xl overflow-hidden border-2 transition-all duration-150 cursor-pointer flex flex-col active:scale-95
                                      ${isSelected ? 'border-[#2B2C86] shadow-md' : isOutOfStock ? 'border-gray-100 opacity-40 cursor-not-allowed' : 'border-gray-200 hover:border-gray-400 hover:shadow-sm'}`}
                                  >
                                    {variant.image_url ? (
                                    ...
                                    <span className={`block text-center text-[11px] font-semibold py-1 px-1 truncate ${isSelected ? 'bg-[#2B2C86] text-white' : 'bg-white text-gray-600'}`}>
                                      <Price amount={variant.price || product.price} />
                                    </span>
                                    {isSelected && (
                                      <span className="absolute top-1 right-1 w-5 h-5 bg-[#2B2C86] rounded-full flex items-center justify-center">
                                ...
                                return (
                                  <button
                                    ...
                                    className={`relative px-4 py-2.5 rounded-xl border-2 text-sm font-medium transition-all duration-150 cursor-pointer select-none active:scale-95
                                      ${isSelected
                                        ? 'border-[#2B2C86] bg-[#2B2C86] text-white shadow-md'
                                        : isOutOfStock
                ...
                <div className="mb-8">
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex flex-col">
                      ...
                      {activeStock > 0 ? (
                        <span className="text-[#E6308A] font-medium text-sm">
                ...
                <div className="flex flex-col sm:flex-row gap-4 mb-8">
                  <button
                    ...
                    className={`flex-1 bg-[#2B2C86] hover:bg-[#222370] text-white py-4 rounded-lg font-semibold transition-colors flex items-center justify-center space-x-2 text-lg whitespace-nowrap cursor-pointer ${(activeStock === 0 || needsVariantSelection || needsColorSelection) ? 'opacity-50 cursor-not-allowed' : ''}`}
                  ...
                  {activeStock > 0 && !needsVariantSelection && !needsColorSelection && (
                    <button
                      ...
                      className="sm:w-auto bg-[#2B2C86] hover:bg-[#222370] text-white px-8 py-4 rounded-lg font-semibold transition-colors whitespace-nowrap cursor-pointer"
                    >
                      Buy Now
                    </button>
                  )}
                ...
                {['description', 'features', 'care', 'reviews'].map((tab) => (
                  <button
                    ...
                    className={`pb-4 font-semibold transition-colors relative whitespace-nowrap cursor-pointer ${activeTab === tab
                      ? 'text-gray-900 border-b-2 border-[#2B2C86]'
