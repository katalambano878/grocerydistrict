...
function OrderTrackingContent() {
  ...
  const getStatusBadge = () => {
    ...
    const statusMap: Record<string, { label: string; color: string }> = {
      'pending': { label: 'Pending', color: 'bg-[#FCE7F3] text-[#2B2C86]' },
      'processing': { label: 'Processing', color: 'bg-[#E6308A]/10 text-[#E6308A]' },
      'shipped': { label: 'Packaged', color: 'bg-[#E6308A]/10 text-[#E6308A]' },
      'dispatched_to_rider': { label: 'Dispatched To Rider', color: 'bg-[#E6308A]/10 text-[#E6308A]' },
          ...
          <div className="bg-white rounded-xl shadow-sm p-8">
            <form onSubmit={handleTrack} className="space-y-6">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-[#2B2C86] focus:border-[#2B2C86]"
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-[#2B2C86] hover:bg-[#222370] text-white py-4 rounded-lg font-semibold transition-colors whitespace-nowrap disabled:opacity-50"
              >
                {loading ? (
            ...
            <div className="mt-8 p-4 bg-[#E6308A]/5 border border-[#E6308A]/20 rounded-lg">
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-[#2B2C86]">Need Help?</p>
                  <p className="text-sm text-[#E6308A] mt-1">
      ...
      <div className="max-w-4xl mx-auto">
                ...
                <div className="relative flex flex-col items-center mr-6">
                  ...
                  {index < trackingSteps.length - 1 && (
                    <div className={`w-0.5 h-16 mt-2 ${
                      step.status === 'completed' ? 'bg-[#2B2C86]' : 'bg-gray-200'
                    }`}></div>
                  )}
                </div>
          ...
          <div className="space-y-4">
            {order.order_items?.map((item: any) => (
              <div key={item.id} className="flex items-center space-x-4 p-4 bg-gray-50 rounded-lg">
                ...
                <p className="font-bold text-gray-900">GH₵ {Numbermoney(item.unit_price)}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-8 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Link href="/contact" className="text-[#2B2C86] hover:text-[#222370] font-semibold whitespace-nowrap">
              <i className="ri-customer-service-line mr-1"></i>
              Contact Support
            </Link>
            <Link href="/returns" className="text-[#2B2C86] hover:text-[#222370] font-semibold whitespace-nowrap">
    ...
    <Suspense fallback={
      <main className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="w-12 h-12 border-4 border-[#2B2C86] border-t-transparent rounded-full animate-spin mx-auto"></div>
        </div>
      </main>
    }>
