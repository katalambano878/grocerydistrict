...
const LLM_TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Search the website\'s pages and content for information. Use this to answer ANY question about the business, policies, how things work, FAQs, contact info, shipping, returns, payment methods, account management, checkout process, blog content, or anything else about Grocery District. This searches all public pages of the website. ALWAYS use this tool when a customer asks about the business, policies, processes, or anything non-product related.',
      parameters: {
        type: 'object',
        properties: {
...
function buildSystemPrompt(profile: ChatCustomerProfile | null, pagePath?: string): string {
  ...
  let prompt = `You are the AI shopping assistant for Grocery District — a supermarket and imported-goods retailer & wholesaler based in Ashongman Estate, Accra, Ghana. We stock UK and USA groceries, baby groceries (baby food, formula and more), detergent and household products, and skincare. We serve both wholesale and retail customers and deliver across Accra and nationwide in Ghana (in-store shopping also available). We are open 7 days a week, Monday–Saturday 8:30am–8:00pm. Today is ${now}.
...
2. ALWAYS provide the customer with direct contact information for faster help. Share: Phone/WhatsApp +233 55 297 1577 (https://wa.me/233552971577), email hello@grocerydistrict.shop, Instagram @grocery_district1, or visit Ashongman Estate, Accra. Say something like: "I've created a support ticket for you. For a faster response, you can also reach us at +233 55 297 1577 (call or WhatsApp) or hello@grocerydistrict.shop."
...
- Total spent: GH₵${money(profile.total_spent)}
...
async function handleWithAI(
  ...
  if (cartItems && cartItems.length > 0) {
    ...
    systemPrompt += `\n\nCUSTOMER'S CURRENT CART (${cartItems.length} item${cartItems.length > 1 ? 's' : ''}, subtotal GH₵${money(cartTotal)}):`;
    for (const item of cartItems) {
      systemPrompt += `\n- ${item.name} × ${item.quantity} = GH₵${money(item.price * item.quantity)} (ID: ${item.id})`;
    }
  ...
  try {
    ...
    if (!assistantContent) {
      if (ticketCard) {
        ...
        assistantContent = `I'm sorry, I wasn't able to process that properly. You can try rephrasing your request, or for immediate help reach us at 000 000 0000 (call or WhatsApp) or hello@grocerydistrict.shop. Our team is available Mon-Sat, 9am-6pm WAT.`;
      }
    }
    ...
    case 'create_order': {
      ...
      if (orderResult.paymentUrl) {
        result.paymentAction = {
          ...
          orderNumber: orderResult.orderNumber,
          label: `Pay GH₵${money(orderResult.total)} Now`,
        };
      }
