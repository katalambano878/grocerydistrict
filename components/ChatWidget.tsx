...
const WIDGET_TITLE = "Grocery District";
...
const STATUS_COLORS: Record<string, string> = {
  pending: 'bg-[#FCE7F3] text-[#2B2C86]',
  processing: 'bg-[#E6308A]/10 text-[#E6308A]',
  shipped: 'bg-[#E6308A]/10 text-[#E6308A]',
  dispatched_to_rider: 'bg-[#E6308A]/10 text-[#E6308A]',
  delivered: 'bg-gray-100 text-gray-900',
  cancelled: 'bg-[#FFCCCC] text-[#9A1900]',
  open: 'bg-[#E6308A]/10 text-[#E6308A]',
  in_progress: 'bg-[#FCE7F3] text-[#2B2C86]',
    ...
    <>
      ...
      {!open && (
        <button
          ...
          className="fixed bottom-[5.5rem] right-3 z-[9999] w-14 h-14 rounded-full bg-gray-700 hover:bg-[#222370] text-white shadow-2xl flex items-center justify-center transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2 sm:bottom-6 sm:right-4"
            ...
            <div className="border-t border-gray-100 p-3 sm:p-4 bg-gradient-to-r from-gray-100 to-gray-50 flex-shrink-0 space-y-2.5">
              ...
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button key={star} type="button" onClick={() => setFeedbackRating(star)}
                    className={`text-3xl sm:text-2xl transition-transform hover:scale-110 active:scale-95 ${star <= feedbackRating ? 'text-[#E6308A]' : 'text-gray-300'}`}>
              ...
              <div className="flex gap-2">
                <button type="button" onClick={submitFeedback} disabled={feedbackRating === 0}
                  className="px-4 py-2 text-sm font-medium bg-gray-700 text-white rounded-lg hover:bg-[#222370] disabled:opacity-40 transition-colors active:scale-95">
                  Submit &amp; Clear
                </button>
              ...
              <form onSubmit={(e) => { e.preventDefault(); send(); }} className="flex gap-2">
                ...
                {input.trim() ? (
                  <button
                    type="submit"
                    disabled={loading}
                    className="shrink-0 w-11 h-11 sm:w-10 sm:h-10 rounded-xl bg-gray-700 hover:bg-[#222370] text-white flex items-center justify-center disabled:opacity-40 disabled:pointer-events-none transition-all active:scale-95"
                    aria-label="Send"
                  >
      ...
      <div className={`max-w-[85%] sm:max-w-[88%] space-y-2`}>
        ...
        {message.actions?.filter(a => a.type === 'payment_link').map((a, idx) => (
          <a
            ...
            className="flex items-center justify-center gap-2 w-full px-4 py-3 bg-gray-700 hover:bg-[#222370] text-white font-semibold rounded-xl shadow-md transition-all active:scale-[0.98] text-sm"
          >
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="flex items-center gap-2.5 p-2.5 sm:p-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm font-bold text-gray-600">GH₵{money(product.price)}</p>
        ...
        <div className="flex flex-col gap-1.5 flex-shrink-0">
          {product.inStock && (
            <button
              ...
              className="px-2.5 sm:px-3 py-1.5 bg-gray-700 hover:bg-[#222370] text-white text-xs font-semibold rounded-lg transition-all active:scale-95"
            >
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="px-4 py-2 space-y-1">
        {order.items.slice(0, 3).map((item, i) => (
          <div key={i} className="flex justify-between text-xs">
            ...
            <span className="text-gray-900 font-medium ml-2">GH₵{money(item.price)}</span>
          </div>
        ))}
      ...
      <div className="px-4 py-2 border-t border-gray-50 flex justify-between items-center">
        ...
        <span className="text-sm font-bold text-gray-900">GH₵{money(order.total)}</span>
      </div>
...
function TicketCard({ ticket }: { ticket: ChatTicket }) {
  return (
    <div className="bg-white rounded-xl border border-[#E6308A]/20 shadow-sm overflow-hidden">
      <div className="px-4 py-3 bg-[#E6308A]/5">
        <div className="flex items-center gap-2 mb-1">
          <i className="ri-customer-service-2-line text-[#E6308A]" />
          <span className="text-xs font-bold text-[#E6308A]">Support Ticket Created</span>
        </div>
        <div className="flex items-center justify-between">
          <p className="text-sm font-medium text-gray-900">{ticket.subject}</p>
          <span className="text-xs font-mono text-[#E6308A]">#{ticket.ticket_number}</span>
        </div>
...
function ReturnCard({ ret }: { ret: ChatReturn }) {
  return (
    <div className="bg-white rounded-xl border border-[#E6308A]/30 shadow-sm overflow-hidden">
      <div className="px-4 py-3 bg-[#FCE7F3]/50">
        <div className="flex items-center gap-2 mb-1">
          <i className="ri-arrow-go-back-line text-[#E6308A]" />
          <span className="text-xs font-bold text-[#2B2C86]">Return Request Submitted</span>
        </div>
    ...
    <div className={`rounded-xl border shadow-sm overflow-hidden ${coupon.valid ? 'bg-white border-gray-100' : 'bg-white border-[#FF6666]/20'}`}>
      <div className={`px-4 py-3 ${coupon.valid ? 'bg-gray-100/50' : 'bg-[#FFCCCC]/50'}`}>
        ...
        <div className="flex items-center justify-between">
          ...
          {coupon.valid && coupon.value && (
            <span className="text-sm font-bold text-gray-600">
              {coupon.type === 'percentage' ? `${coupon.value}% OFF` : coupon.type === 'free_shipping' ? 'Free Shipping' : `GH₵${money(coupon.value)} OFF`}
            </span>
          )}
        </div>
        ...
        {coupon.valid && coupon.minimum_purchase && (
          <p className="text-[10px] text-gray-400 mt-1">Min. purchase: GH₵{money(coupon.minimum_purchase)}</p>
        )}
