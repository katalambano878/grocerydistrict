#!/usr/bin/env python3
"""Restore Grocerydistrict dump into grocerydistrict_staging on fleet-postgres."""
import json
import os
import subprocess
import sys
...
DUMP = Path(os.environ.get("GD_DUMP_DIR", "/data/coolify/grocerydistrict-staging/dumps"))
DB = os.environ.get("GD_DB_NAME", "grocerydistrict_staging")
ROLE = os.environ.get("GD_DB_ROLE", DB)
...
def rewrite_storage_urls():
    ...
    for table, col in patterns:
        try:
            ...
            pass
    for table in ("site_settings", "store_settings", "cms_content"):
        try:
            psql(
                f"""
                UPDATE public.{table}
